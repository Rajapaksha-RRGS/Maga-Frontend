 ...
 * Follows the same pattern as labour assign but for operator roles.
 */
import { useState } from 'react';
import { AlertTriangle, Search, UserPlus, Users, Cog, Calendar, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function OperatorAssignmentsPage() {
  ...
  return (
    <div className="px-4 md:px-6 py-5">
      ...
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg bg-blue-50 border border-blue-200 flex items-center justify-center">
            <Cog size={16} className="text-blue-700" />
          </div>
          <div>
            <h1 className="text-base font-medium text-slate-800">Operator assign</h1>
            <p className="text-xs text-slate-400 mt-0.5">Assign operators to supervisors for daily operations</p>
          </div>
        </div>
      </div>
      ...
      {/* Loading */}
      {isLoading && (
        <p className="text-sm text-slate-400 py-8 text-center">Loading operator assignments…</p>
      )}
          ...
          <div className="bg-white border border-slate-200 rounded-lg">
            <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-medium text-slate-700">Unassigned operators</h2>
              </div>
              <span className="text-xs text-slate-400 bg-slate-50 border border-slate-200 px-2 py-1 rounded-md">0 operators</span>
            </div>
            <div className="p-4">
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="text-sm text-slate-400 mb-1">No unassigned operators</p>
                <p className="text-xs text-slate-300">All operators have been assigned for {selectedDate}</p>
              </div>
            </div>
          </div>
      ...
      {/* Info banner */}
      <div className="flex items-center gap-2 bg-blue-50 border border-blue-200 rounded-lg px-4 py-3 mt-5">
        <AlertTriangle size={16} className="text-blue-500 flex-shrink-0" />
        <p className="text-sm text-blue-700">
          Operator assignments will be linked to the backend API. Use the same date-based workflow as labour assign.
        </p>
      </div>
    </div>
  );
